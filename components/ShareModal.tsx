  ...
  const handleFinalShare = async () => {
    ...
    try {
      ...
      if (canShare && navigator.share) {
        // Some browsers fail if you include too many fields with files.
        // We'll prioritize the file.
        await navigator.share({
          files: [preparedFile],
          title: `Audit: ${data.siteName}`,
          text: `Industrial Site Audit for ${data.siteName}`
        });
        onClose();
      } else {
        // Fallback: Trigger download immediately if sharing isn't supported
        downloadFile(preparedFile);
        setError("System share sheet not supported on this browser. File has been downloaded to your device instead.");
        setStage('ERROR');
      }
      ...
      // Automatic Fallback: If the share sheet fails for any reason (permission, block, etc)
      // we just give them the file.
      downloadFile(preparedFile);
      setError("The system share sheet failed or was blocked. The report has been downloaded to your device instead.");
      setStage('ERROR');
    }
  };
          ...
          {stage === 'READY' && (
            <div className="py-6 flex flex-col items-center text-center space-y-6 animate-in zoom-in-95 duration-300">
              ...
              <button 
                ...
                className={`w-full py-5 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white flex items-center justify-center gap-3 font-black text-xs tracking-[0.2em] shadow-xl shadow-indigo-500/20 active:scale-95 transition-all`}
              >
          ...
          {stage === 'ERROR' && (
            <div className="space-y-4">
              <div className={`p-4 rounded-2xl flex items-start gap-3 border ${darkMode ? 'bg-red-950/20 border-red-500/30 text-red-200' : 'bg-red-50 border-red-200 text-red-800'}`}>
                ...
                <div className="space-y-1">
                  <p className="text-xs font-black uppercase tracking-tight">Report Status</p>
              ...
              {preparedFile && (
                <button 
                  onClick={() => downloadFile(preparedFile)}
                  className={`w-full p-4 rounded-xl flex items-center justify-center gap-2 font-black text-[10px] uppercase tracking-widest border transition-colors ${darkMode ? 'border-blue-500/50 text-blue-400 bg-blue-500/10 hover:bg-blue-500/20' : 'border-blue-200 text-blue-600 bg-blue-50 hover:bg-blue-100'}`}
                >
                  <Download size={14} /> Download File Locally
                </button>
              )}
